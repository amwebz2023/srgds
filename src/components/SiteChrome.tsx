...
export function SiteChrome({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  useEffect(() => { const onScroll = () => setShowTop(window.scrollY > 340); window.addEventListener("scroll", onScroll); fetch("/api/auth/session").then((response) => response.json()).then((result) => setAuthenticated(result.authenticated === true)).finally(() => setSessionLoaded(true)); return () => window.removeEventListener("scroll", onScroll); }, []);
